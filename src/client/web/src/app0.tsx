...
const ImageConverter = () => {
  ...
  const onDrop = useCallback((acceptedFiles: any) => {
    const newFiles = acceptedFiles.slice(0, 5).map(file => Object.assign(file, {
  ...
  const pollForCompletion = async (token) => {
    ...
    return new Promise((resolve) => {
      const interval = setInterval(async () => {
        ...
        if (status === 'completed') {
          ...
          const imagesResponse = await fetch('http://localhost:8080/api/download-images-path', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto mt-8">
      <CardHeader>
        <CardTitle className="text-2xl font-bold text-center">Image Format Converter</CardTitle>
      </CardHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {convertedImages.map((image) => (
                <div key={image} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
